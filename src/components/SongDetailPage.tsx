...
const SongDetailPage = () => {
  ...
  // 스와이프 속도 관련 상태 추가
  const [swipeTimes, setSwipeTimes] = useState<{time: number, position: number}[]>([]);
  ...
  useEffect(() => {
    ...
    if (location.state && typeof location.state === 'object') {
      ...
      if ('results' in location.state && Array.isArray(location.state.results)) {
  ...
  const canNavigateNext = songList.length > 0 && currentIndex < songList.length - 1;
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    ...
    const now = Date.now();
    setSwipeTimes(prev => {
      const updated = [...prev, { time: now, position: currentX }]
        .filter(item => now - item.time <= maxSwipeTime);
      return updated;
    });
    ...
    if (canLeftSwipe || canRightSwipe) {
      ...
      const moveRatio = Math.min(Math.abs(diff) / screenWidth.current, maxMoveRatio);
  ...
  const handleTouchEnd = () => {
    ...
    const isSwipeGesture = absDistance > minSwipeDistance || absVelocity > minSwipeVelocity;
  ...
  // 특정 위치로 애니메이션
  const animateToPosition = (targetPosition: number, duration: number = 300) => {
